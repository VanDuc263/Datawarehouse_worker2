...
def main():
    ...
    try:
        ...
        if fs.exists(status_file):
            ...
            if len(clean_status)==0 or clean_status[0] != "P3":
                logger.info(f"🔹 {clean_file} chưa P3 → dw_load_dim dừng.")
                return
        else:
            logger.info("🔹 Chưa có file_status.csv → dw_load_dim dừng.")
            return
    except Exception as e:
        logger.error(f"❌ Lỗi đọc file_status.csv: {e}")
        return

    logger.info(f"🔹 {clean_file} đã P3 → Bắt đầu dw_load_dim")
    ...
    try:
        # ======================== Đọc clean_data
        df = pd.read_csv(clean_path, storage_options=MINIO_STORAGE_OPTIONS)
        ...
        # ======================== Tạo dim_brand
        dim_brand = df[["product_name"]].copy()
        dim_brand["brand"] = dim_brand["product_name"].apply(lambda x: x.split()[0])
        dim_brand = dim_brand[["brand"]].drop_duplicates().reset_index(drop=True)
        dim_brand["brand_id"] = dim_brand.index + 1

        # Lưu lên MinIO tạm
        dim_brand.to_csv(dim_path, index=False, encoding="utf-8-sig", storage_options=MINIO_STORAGE_OPTIONS)
        logger.info(f"✅ Tạo xong dim_brand: {dim_path}")
        ...
        dim_brand.to_sql(table_name, con=engine, if_exists="replace", index=False)
        logger.info(f"🎯 Load dim_brand thành công vào bảng `{table_name}`")
        ...
        logger.error(f"❌ Lỗi dw_load_dim: {e}")
